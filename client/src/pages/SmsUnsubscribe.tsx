...
export default function SmsUnsubscribe() {
  ...
  // State
  const [token, setToken] = useState<string | null | undefined>(undefined); // undefined = not loaded, null = loaded but missing
  ...
  // Get token from URL (client-side only to avoid SSR issues)
  useEffect(() => {
    if (typeof window !== 'undefined') {
      const params = new URLSearchParams(window.location.search);
      setToken(params.get('token')); // Will be string or null
    }
  }, []);
  
  // Verify token once loaded
  useEffect(() => {
    // Wait until token has been loaded from URL
    if (token === undefined) {
      return; // Still loading, stay in verifying state
    }
    
    if (!token) {
      setStep('error');
      setErrorMessage('No unsubscribe token provided. Please use the link from your text message.');
      return;
    }
    
    // Fetch phone from token (verify it)
    fetch('/api/sms-unsubscribe/verify', {
      method: 'POST',
          ...
          <>
            <CardHeader>
              <div className="flex items-center gap-2 mb-2">
                ...
                <CardTitle>Unsubscribe from Text Messages</CardTitle>
              </div>
              <CardDescription>
                We're sorry to see you go! You can unsubscribe from all Julie's Family Learning Program text messages below.
              </CardDescription>
            </CardHeader>
            ...
            <CardContent className="space-y-6">
              {phone && (
                <div className="p-3 bg-muted rounded-md">
                  ...
                  <p className="font-medium" data-testid="text-unsubscribe-phone">{phone}</p>
                </div>
              )}
              ...
              <div className="p-3 bg-blue-50 dark:bg-blue-950 border border-blue-200 dark:border-blue-800 rounded-md">
                <p className="text-sm text-blue-900 dark:text-blue-100">
                  <strong>Alternative:</strong> You can also reply with "STOP" to any text message from us to unsubscribe.
                </p>
              </div>
            </CardContent>
        ...
        {step === 'success' && (
          <>
            <CardHeader>
              ...
              <CardDescription>
                You will no longer receive text messages from Julie's Family Learning Program at {phone}.
              </CardDescription>
            </CardHeader>
            ...
            <CardContent>
              <div className="p-4 bg-muted rounded-md space-y-2">
                <p className="text-sm text-muted-foreground">
                  Changed your mind? You can always subscribe again by contacting us or visiting our website.
                </p>
              </div>
            </CardContent>
