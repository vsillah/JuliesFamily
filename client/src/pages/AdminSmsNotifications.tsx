...
import { Plus, Edit, Trash2, MessageSquare, Send, Clock, BarChart3 } from "lucide-react";
import type { SmsTemplate, SmsSend, SmsBulkCampaign } from "@shared/schema";
...
export default function AdminSmsNotifications() {
  ...
  const [activeTab, setActiveTab] = useState<"templates" | "send" | "bulk-send" | "campaigns" | "history">("templates");
  ...
  const { data: bulkCampaigns = [], isLoading: campaignsLoading } = useQuery<SmsBulkCampaign[]>({
    queryKey: ['/api/sms/bulk'],
          ...
          {campaignsLoading ? (
            ...
            <Card>
              <CardContent className="py-12 text-center">
                ...
                <h3 className="text-lg font-medium mb-2">No bulk campaigns yet</h3>
                <p className="text-muted-foreground">
                  Bulk campaigns will appear here
                </p>
              </CardContent>
            </Card>
          ) : (
            <div className="space-y-4">
              {bulkCampaigns.map((campaign) => {
                ...
                return (
                  <Card key={campaign.id} data-testid={`card-bulk-campaign-${campaign.id}`}>
                    <CardHeader>
                      <div className="flex items-start justify-between gap-4">
                        <div className="flex-1 min-w-0">
                          ...
                          <CardDescription className="space-y-1 mt-2">
                            <div className="flex flex-wrap gap-2">
                              {campaign.personaFilter && (
                                <Badge variant="outline" className="text-xs" data-testid={`badge-persona-${campaign.id}`}>
                                  Persona: {campaign.personaFilter}
                                </Badge>
                              )}
                              {campaign.funnelStageFilter && (
                                <Badge variant="outline" className="text-xs" data-testid={`badge-funnel-${campaign.id}`}>
                                  Stage: {campaign.funnelStageFilter}
                                </Badge>
                              )}
                            </div>
                        ...
                        <Badge variant={statusVariant} data-testid={`text-campaign-status-${campaign.id}`}>
                          {campaign.status === 'sending' && <Clock className="w-3 h-3 mr-1 animate-spin" />}
                          {campaign.status}
                        </Badge>
                      </div>
                    </CardHeader>
                    <CardContent className="space-y-4">
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        ...
                        <div className="text-center" data-testid={`metric-failed-${campaign.id}`}>
                      ...
                      {/* Message Preview */}
                      {(template || campaign.customMessage) && (
                        <div className="pt-4 border-t">
                          <div className="text-sm">
                            ...
                            {template ? (
                              ...
                              <span className="text-muted-foreground">
                                {campaign.customMessage && campaign.customMessage.length > 100
                                  ? `${campaign.customMessage.substring(0, 100)}...`
