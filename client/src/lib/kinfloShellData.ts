...
export type ShellHostedActivationRawPreflightOutputRedactionItem = {
  id: string;
  label: string;
  rawOutputRisk: string;
  sanitizedCommitShape: string;
  reviewerAction: string;
  reviewState: "pending_storage_approval";
};

export type ShellHostedActivationRawPreflightOutputRedactionChecklist = {
  phase: 133;
  ...
  sourceDocuments: string[];
  redactionItems: ShellHostedActivationRawPreflightOutputRedactionItem[];
  commitRules: string[];
  blockedActions: string[];
...
export type ShellClientWebsiteLaunchComposer = {
...
export type ShellClientWebsiteConfigurationSaveRequest = {
  siteKey: string;
  label: string;
...
export type ShellClientWebsiteConfigurationAuditTimeline = {
  siteKey: string;
  label: string;
...
export type ShellClientWebsiteDomainReadinessPacket = {
  siteKey: string;
  label: string;
...
export type ShellClientWebsiteStudio = {
  ...
  launchComposer: ShellClientWebsiteLaunchComposer;
...
const fixtureClientWebsiteStudio: ShellClientWebsiteStudio = {
  ...
  launchComposer: {
    ...
    totalSteps: 12,
    providerBoundary: "The launch composer is a selected-site planning surface only. It does not create tenants, create sites, invite admins, publish pages, write leads, attach domains, run generated API imports, execute hosted Convex, or call providers.",
    rows: [
      {
        siteKey: "julies-family-public",
...
const fixtureHostedSmokeGapBacklog: ShellHostedSmokeGapBacklog = {
  ...
  gaps: [
    ...
    },
    {
      ...
      smokeMode: "read_only",
      localProof: "Phase 160 and Phase 161 map launch composer fixture state and read-only Convex query coverage; Phase 162 makes it adapter-switch evidence.",
      hostedProofRequired: "Run a read-only hosted launch composer query and confirm tenant, template, admin preset, approval evidence, execution steps, blocked live switches, and provider boundaries match the selected client site.",
      rollbackArtifact: "Fixture launch composer remains active if hosted composition state diverges or exposes live execution paths.",
      owner: "platform.super_admin",
      blockedUntil: "Client website launch composer read smoke is approved.",
      canRun: false,
      providerWrites: false,
      liveConvexExecution: false,
    },
...
const fixtureHostedSmokeExecutionSequencer: ShellHostedSmokeExecutionSequencer = {
  ...
  batches: [
    ...
    },
    {
      ...
      requiredBeforeRun: "Client site fixture, permission preset, configuration profile, domain, invitation, launch blueprint, and launch composer owners are confirmed before hosted reads.",
      evidenceTarget: "Capture tenant-scoped site factory reads, owner/invite role mapping, configuration packets, domain/invitation readiness, default pages, launch composer state, permission gates, and publish disabled state.",
      abortCondition: "Stop if site factory reads leak across tenants, configuration or launch packet links do not match the selected site, launch composer state diverges, or publish/invite/save actions become enabled.",
      rollbackPlan: "Keep fixture permission presets, configuration packets, launch blueprints, and launch composer selected until hosted scope and launch packet mapping are accepted.",
      owner: "platform.super_admin",
      blockedUntil: "Client site factory, configuration, launch blueprint, and launch composer read smokes are approved.",
      canRun: false,
      providerWrites: false,
      liveConvexExecution: false,
    },
...
const fixtureHostedSmokeEvidenceLedger: ShellHostedSmokeEvidenceLedger = {
  ...
  entries: [
    ...
    },
    {
      ...
      status: "pending_human_gate",
      functionCount: 14,
      expectedTranscript: "Capture site factory read scope across admin invitation readiness, permission presets, configuration packets, domain readiness, experience presets, launch blueprints, siteFactory.listClientWebsiteLaunchComposer state, owner/invite role mapping, permission gates, and disabled save/publish/invite actions.",
      evidenceSlots: [
        "admin invitation readiness response",
        "admin permission preset response",
        "configuration review and change packet responses",
        "configuration approval, save, audit, rollback, and publish readiness responses",
        "domain readiness and experience preset responses",
      ...
      acceptanceCriteria: [
        "all site factory reads stay tenant and selected-site scoped",
        "configuration, domain, invitation, blueprint, and launch composer rows match the selected client site",
        "save, publish, invite, domain, and launch actions remain gated",
      ],
      abortIf: "Stop if site factory reads leak across tenants, configuration or launch composer state diverges from the selected site, or save/publish/invite/domain/launch actions enable.",
      rollbackReference: "Keep fixture permission presets, configuration packets, domain readiness, invitation readiness, launch blueprints, and launch composer selected until hosted mapping is accepted.",
      owner: "platform.super_admin",
      blockedUntil: "Client site factory, configuration, domain, invitation, launch blueprint, and launch composer read smokes are approved.",
      canRecord: false,
      providerWrites: false,
      liveConvexExecution: false,
    },
...
const fixtureHostedActivationRunbook: ShellHostedActivationRunbook = {
  ...
  rawPreflightOutputRedactionChecklist: {
    ...
    pendingRules: 5,
    acceptedRules: 0,
    nextGate: "After private raw-output storage is approved, review the redaction checklist before any sanitized hosted preflight result is committed.",
    reviewPacketPath: "docs/phase133-hosted-activation-raw-preflight-output-redaction-checklist.md",
    sourceDocuments: [
      "docs/phase126-hosted-activation-preflight-evidence-ledger.md",
      "docs/phase128-hosted-activation-preflight-result-contract.md",
    ...
    redactionItems: [
      {
        id: "secret-like-values",
        label: "Secret-like values",
        rawOutputRisk: "Tokens, API keys, auth issuer secrets, client secrets, copied 1Password fields, or shell-exported values may appear in raw output.",
        sanitizedCommitShape: "Commit only yes/no or enum status fields; never commit the value, prefix, suffix, hash, or masked token.",
        reviewerAction: "Confirm raw output stayed in the approved private surface and that committed notes contain no secret-like fragments.",
        reviewState: "pending_storage_approval",
      },
      {
        id: "local-private-paths",
        label: "Local private paths",
        rawOutputRisk: "Stack traces, command output, or local config checks may reveal machine paths, usernames, project folders, or private artifact locations.",
        sanitizedCommitShape: "Commit only the high-level result and a generic note such as local env present or local artifact reviewed.",
        reviewerAction: "Remove or generalize local path details before any source-controlled summary is written.",
        reviewState: "pending_storage_approval",
      },
      {
        id: "hosted-provider-identifiers",
        label: "Hosted provider identifiers",
        rawOutputRisk: "Hosted Convex deployment names, dashboard URLs, auth issuer URLs, client ids, provider request ids, or resource ids may appear.",
        sanitizedCommitShape: "Commit only boolean visibility/touched fields and the approved enum result status.",
        reviewerAction: "Verify provider identifiers remain in the private raw-output store and do not enter docs, chat, generated artifacts, or deploy logs.",
        reviewState: "pending_storage_approval",
      },
      {
        id: "stack-trace-private-context",
        label: "Stack trace private context",
        rawOutputRisk: "Failure traces can combine private paths, dependency versions, env names, and command arguments in one block.",
        sanitizedCommitShape: "Commit only blocked or aborted plus a short sanitized reason that does not include raw stack frames.",
        reviewerAction: "Summarize failure class and next gate without copying raw trace lines.",
        reviewState: "pending_storage_approval",
      },
      {
        id: "mutation-or-provider-output",
        label: "Mutation or provider output",
        rawOutputRisk: "Any nonzero external write count, mutation response, provider response, or created resource id proves the run crossed the provider boundary.",
        sanitizedCommitShape: "Commit externalWrites as 0 only for prepare-only proof; otherwise stop and keep the raw output private for owner review.",
        reviewerAction: "Block codegen, live smoke, adapter switch, provider writes, and client launch until the owner reviews the private output.",
        reviewState: "pending_storage_approval",
      },
    ],
    commitRules: [
      "Commit only sanitized yes/no, number, or enum fields from the approved result contract.",
      "Never commit raw command output, stack traces, provider ids, local paths, secrets, masked secret fragments, or copied dashboard values.",
      "Keep raw output in the owner-approved private storage surface selected after Phase 132.",
      "Treat any nonzero external write, hosted deployment touch, or provider response as a stop condition for owner review.",
      "Do not use this checklist as approval to run hosted preflight, codegen, generated API import, live smoke, adapter switch, or provider writes.",
    ],
    blockedActions: [
      "review raw output before owner approves private storage",
      "redact raw output into a committed result before owner approval",
      "commit raw activation preflight logs",
      "paste secret-bearing output into repo docs, chat, generated artifacts, or deploy logs",
      "enter real hosted Convex or auth env values",
      "run npm run kinflo:activation-preflight against real hosted env values",
      "run npm run convex:codegen",
      "commit generated Convex API files",
