...
4. Run `npm run kinflo:live-handoff`.
5. Configure hosted Convex and auth outside committed source.
6. Run `npm run convex:codegen` only after hosted setup approval.
7. Review generated `api.<module>.<function>` handles against `KINFLO_GENERATED_API_BINDINGS`.
8. Execute the manifest steps in `docs/convex-live-smoke-manifest.json` order.
9. Keep mutation surfaces gated until each step has audit evidence and a rollback note.
