...
| Inventory env vars without printing secrets. | Repo-complete | `npm run kinflo:inventory-env` passes; [docs/phase0-env-inventory.md](phase0-env-inventory.md) records 57 source env keys, 57 documented in `.env.example`, 0 missing. | The script does not read `.env`, `.env.local`, shell values, provider secrets, or deployment state. |
