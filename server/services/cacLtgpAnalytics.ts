...
export class CacLtgpAnalyticsService {
  constructor(private storage: IStorage, private organizationId?: string) {}
  ...
  async getCACLTGPOverview(): Promise<CACLTGPOverview> {
    ...
    const spendResult = this.orgFilter
      ? await db.execute(sql`
          ...
          WHERE organization_id = ${this.orgFilter}
        `)
      : await db.execute(sql`
          SELECT 
            COALESCE(SUM(amount_spent), 0) as total_spend,
    ...
    const ltgpResult = this.orgFilter
      ? await db.execute(sql`
          SELECT 
          ...
          WHERE lifetime_gross_profit > 0 AND organization_id = ${this.orgFilter}
        `)
      : await db.execute(sql`
          SELECT 
            COALESCE(AVG(lifetime_gross_profit), 0) as avg_ltgp,
    ...
    const topChannelsResult = this.orgFilter
      ? await db.execute(sql`
          ...
          channel_economics AS (
            ...
            WHERE de.lifetime_gross_profit > 0 AND de.organization_id = ${this.orgFilter} AND la.organization_id = ${this.orgFilter}
          ...
          WHERE c.is_active = true AND c.organization_id = ${this.orgFilter}
          ORDER BY ratio DESC
          LIMIT 5
        `)
      : await db.execute(sql`
  ...
  async getChannelPerformance(): Promise<ChannelPerformance[]> {
    const result = this.orgFilter
      ? await db.execute(sql`
          ...
          WHERE c.organization_id = ${this.orgFilter}
      ...
      ? await db.execute(sql`
          WITH campaign_spend AS (
            SELECT 
              ...
              SUM(donors_acquired) as donors
            FROM channel_spend_ledger
            WHERE campaign_id IS NOT NULL AND organization_id = ${this.orgFilter}
          ...
          campaign_economics AS (
            ...
            FROM lead_attribution la
            INNER JOIN donor_economics de ON la.lead_id = de.lead_id
            WHERE la.campaign_id IS NOT NULL AND de.lifetime_gross_profit > 0 AND de.organization_id = ${this.orgFilter} AND la.organization_id = ${this.orgFilter}
          ...
          WHERE mc.organization_id = ${this.orgFilter}
          ORDER BY ratio DESC
        `)
      : await db.execute(sql`
          WITH campaign_spend AS (
    ...
    const result = this.orgFilter
      ? await db.execute(sql`
          WITH period_spend AS (
            ...
            FROM channel_spend_ledger
            WHERE period_type = ${periodType} AND organization_id = ${this.orgFilter}
          ...
          period_economics AS (
            ...
            LEFT JOIN lead_attribution la ON la.created_at >= ps.period_start AND la.created_at < ps.period_end AND la.organization_id = ${this.orgFilter}
            LEFT JOIN donor_economics de ON la.lead_id = de.lead_id
            WHERE de.lifetime_gross_profit > 0 AND de.organization_id = ${this.orgFilter}
...
export function createCacLtgpAnalyticsService(storage: IStorage, organizationId?: string): CacLtgpAnalyticsService {
  return new CacLtgpAnalyticsService(storage, organizationId);
}
