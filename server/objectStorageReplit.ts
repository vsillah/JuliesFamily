// Replit Object Storage (GCS via sidecar) - used when OBJECT_STORAGE_PROVIDER is not "s3"
import { Storage, File } from "@google-cloud/storage";
import { Response } from "express";
import { randomUUID } from "crypto";
import type { IStorageFile } from "./objectAcl";
import {
  ObjectAclPolicy,
  ObjectPermission,
  canAccessObject,
...
let replitStorageInstance: Storage | null = null;
...
export class ObjectStorageService {
  ...
  async downloadObject(file: IStorageFile & { createReadStream(): import("stream").Readable }, res: Response, cacheTtlSec: number = 3600) {
    try {
