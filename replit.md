...
- **User Profile Management**: Self-service profile update system allowing users to manage their own profile information. Features include: (1) Profile page at /profile with React Hook Form-based UI for updating firstName, lastName, and profileImageUrl, (2) Data integrity protection preventing OIDC login from overwriting existing user data, (3) PATCH /api/user/profile endpoint with authentication, Zod schema validation (updateUserProfileSchema), and field whitelisting for self-service updates, (4) storage.updateUser enhancement with actorId parameter and comprehensive audit logging for all profile changes, (5) Accessible via navigation dropdown menu under "Profile Settings". All profile updates are validated, restricted to safe fields (firstName, lastName, profileImageUrl), and logged to audit_logs table with actorId for accountability.
...
- **Automated Backup Scheduler**: Background service enabling scheduled recurring backups with configurable retention policies. Features include: (1) Database-driven scheduling using setInterval polling (60-second intervals) with schedule types: daily, weekly (specific day of week), monthly (specific day of month, correctly handles days 29-31 by using last valid day in shorter months), (2) Timezone-aware next-run calculation using date-fns-tz with IANA timezone support defaulting to America/New_York, (3) Concurrency control via isRunning flag and lockedUntil timestamp (30-minute timeout) preventing race conditions and detecting stuck jobs, (4) Consecutive failure tracking with lastRunStatus and lastRunError for monitoring, (5) Per-schedule retention enforcement via cleanupOldBackupsBySchedule removing backups beyond retentionCount, (6) Graceful shutdown handlers (SIGINT/SIGTERM) in server lifecycle, (7) Admin-only API endpoints (GET/POST/PATCH/DELETE /api/admin/backup-schedules) with Zod validation using insertBackupScheduleSchema, (8) Tabbed UI in AdminBackups page with ScheduledBackupsTab component featuring schedule creation dialog (table, schedule type, hour/minute, day of week/month, retention count), active/paused toggle, delete confirmation, and real-time schedule list with next-run timestamps. Schedule metadata stored in backupSchedules table; actual backups created in backupSnapshots following existing backup system. Service module located at server/services/backupScheduler.ts with init() called from server/index.ts after route setup.
- **Storage Monitoring Dashboard**: Real-time database storage monitoring system designed to prevent exceeding Replit's 10 GiB PostgreSQL storage limit. Features include: (1) Backend API endpoint GET /api/admin/backup-storage-metrics that queries PostgreSQL system catalogs (pg_total_relation_size) to calculate actual table sizes and current database usage, (2) Projection logic that estimates future storage consumption based on active backup schedules using formula: projected_backup_bytes = Σ(table_size × retention_count) for each active schedule, (3) StorageGauge component displaying visual progress bar with color-coded status alerts (green <70%, yellow 70-90%, red >90%), (4) Real-time metrics showing current usage, projected backup storage, total projected usage, and available space with human-readable formatting (MB, GB), (5) Table breakdown listing top 5 tables by estimated backup storage with schedule counts, (6) Dynamic limit display using formatBytes(metrics.limitBytes) instead of hardcoded values, (7) Smart percentage formatting with formatPercent helper showing 2 decimal places for small values (<0.1%) and 1 decimal place otherwise to prevent misleading "0.0%" displays. Located at top of AdminBackups page for at-a-glance visibility across all tabs. Admin-only access via isAdmin middleware.
