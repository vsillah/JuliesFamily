...
- **Persona×Journey Matrix Grid**: A visual interface for configuring content visibility across 120 permutations, enabling inline editing and A/B test variant management. Supports two content creation methods: (1) Direct assignment via content dialogs - admins can select persona and journey stage when creating/editing content through dropdown selectors, automatically creating contentVisibility records; (2) Matrix-driven creation - empty cells display "Create [ContentType]" buttons with "+" icons that open pre-populated creation dialogs with persona and journey stage already selected. Both methods create contentVisibility records linking content to specific persona×journey combinations. Content list items display assignment badges showing all persona×journey combinations (e.g., "Parents & Families × Awareness", "Students × Consideration").
...
- **Social Media Feed**: Carousel-based display of curated social media posts supporting Instagram, Facebook, and LinkedIn platforms. Features responsive layout (1/2/3 posts per viewport), platform badges on each post, icon-only social links with "Follow us" label, self-contained data fetching, graceful handling of posts without images, and full Content Manager integration for visibility controls, platform selection, and content editing.
- **AI-Powered Screenshot Analysis for Social Media**: A streamlined content entry workflow that uses Google's Gemini AI API (free tier: 250 requests/day) to automatically extract post metadata from screenshots. Admins upload a screenshot of a social media post, and the system analyzes it to extract caption text, detect platform (Instagram/Facebook/LinkedIn), identify username or company name, generate a suggested title, and construct profile links. The extracted data auto-populates form fields in the Content Manager, which admins can review and edit before saving. Features include: automatic MIME type detection (supports JPEG, PNG, WebP), base64 image encoding for API transmission, graceful error handling with toast notifications, screenshot preview display, manual editing capabilities, and integration with both create and edit dialogs in the Social Media content tab. Backend endpoint `/api/analyze-social-post` processes uploads and returns structured JSON data. Uses Google GenAI SDK with `GOOGLE_API_KEY` from Replit Secrets.
- **YouTube Video Integration**: A zero-cost video hosting solution using YouTube embeds with react-lite-youtube-embed for 10x faster loading and enhanced privacy. Videos are stored in the contentItems table with type='video' and metadata containing videoId and category (student_story, virtual_tour, program_highlight). The Admin Content Manager includes a dedicated Videos tab where admins can create/edit videos by entering YouTube URLs (full URL or video ID), which are automatically parsed to extract the video ID. The system features: StudentStoriesCarousel component on the homepage displaying student testimonial videos in a responsive carousel (1/2/3 videos per viewport), VirtualTour dedicated page (/virtual-tour) showcasing facility tour videos with welcome section and categorized displays, VideoEmbed component with privacy-enhanced mode (youtube-nocookie.com) and 16:9 responsive aspect ratio, full integration with the persona-based visibility system via contentVisibility records, navigation links to Virtual Tour in both desktop and mobile menus, and line-item visibility controls with visual distinction for hidden items. Videos query `/api/content/visible/video` for public display and `/api/content/type/video` for admin management.
- **Expandable Program Detail Dialogs**: Full-screen dialog modals that provide comprehensive information about Julie's three core programs (Adult Education, Children's Services, Workforce Development) without creating separate pages. Maintains the modular, card-based design language of the homepage by using shadcn Dialog component. Each dialog features: hero image with dark gradient overlay (black/80→black/20) for text readability, quick stats cards displaying age range/schedule/location/cost, tabbed content sections (Overview, What's Included with checkmark feature list, How to Enroll with numbered steps plus embedded LeadCaptureForm, FAQ accordion), horizontal scrollable tabs on mobile, smooth open/close animations, and keyboard accessibility. Program details are stored in the contentItems table with type='program_detail' and structured metadata (programId, overview, stats array, features array, enrollment steps, FAQs, defaultPersona). Services component fetches program_detail data via `/api/content/type/program_detail` and maps service titles to program IDs through SERVICE_TO_PROGRAM_MAP constant. "Learn More" buttons on service cards open the corresponding dialog. The LeadCaptureForm is embedded in the "How to Enroll" tab with persona pre-population based on program type. Database seeded via `scripts/seedProgramDetails.ts`.
- **Stripe Donation System**: Secure payment processing for one-time donations using Stripe Payment Intents with Elements UI. The system features: dedicated Donate page (/donate) with 6 preset amounts ($25/$50/$100/$250/$500/$1000) plus custom amounts, donor information capture (name, email, phone), anonymous donation option, success page with receipt details, and integration into desktop/mobile navigation. Backend implements secure webhook handling with required signature verification using rawBody, idempotency checks to prevent double-processing on retries, and automatic donation status tracking (pending/succeeded/failed). Donations are stored in the donations table with Stripe payment intent IDs for reconciliation. Frontend converts dollar amounts to cents before API calls; backend uses amounts directly. Stripe credentials (STRIPE_SECRET_KEY, VITE_STRIPE_PUBLIC_KEY) are hot-swappable via Replit Secrets, with separate testing keys for development. Future enhancements: Stripe Subscriptions for true recurring donations, email automation for thank-you messages and receipts, CRM integration to link donations with leads.
...
- **react-lite-youtube-embed**: Lightweight YouTube embed component for fast loading and privacy.