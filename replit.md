...
-   **Uniform Conditional Rendering System**: All matrix-controlled sections (services, lead-magnet, impact, testimonials, events, donation) implement consistent visibility logic using useContentAvailability hook. Features DEFAULT_SECTIONS fallback during loading, nullish coalescing for safe data merging, section wrappers with data-testid attributes for testing, and synchronized query patterns ensuring PersonalizedLeadMagnet matches section visibility requirements (persona-only enabling).
