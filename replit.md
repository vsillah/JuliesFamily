...
-   **Admin Impersonation System**: Production-ready user impersonation with session tracking, allowing admins to preview the site as any user for troubleshooting. Includes mobile-friendly user search (UserSearchCommand), consolidated admin preview dropdown, clear/deselect UI, session persistence in database, and proper session lifecycle management via RESTful endpoints (POST /api/admin/impersonation/start, DELETE /api/admin/impersonation/end/:sessionId, GET /api/admin/impersonation/session).
