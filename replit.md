...
-   **Layout/Theme System with AI Recommendations**: Fixed template system with 4 visual layout options (Classic, Nature, Modern, Community) selected during organization provisioning (Step 3). Each layout defines distinct visual styling for hero components, color schemes, typography, and imagery patterns. Layout field stored in organizations table with 'classic' default. Layout-specific hero components (NatureHero, ModernHero, CommunityHero) dynamically rendered on Home page based on organization.layout. Full type safety maintained using OrganizationLayout enum from shared schema. Layout data flows from database → API → OrganizationContext → components with proper type preservation in all org-switching flows. **Comprehensive Theme Application**: LayoutProvider applies theme-specific CSS classes (`theme-classic`, `theme-nature`, `theme-modern`, `theme-community`) to document root. Each theme defines CSS variables for section padding, card borders/shadows, border radius, and accent opacity. Layout-aware utility classes (`layout-card`, `layout-section`, `layout-accent`) automatically adapt styling based on active theme. Theme characteristics: Classic (elevated cards, comfortable spacing, rounded borders, bold accents), Nature (outlined cards, spacious sections, organic borders, natural accents), Modern (minimal cards, compact spacing, sharp borders, vibrant accents), Community (warm cards, comfortable spacing, soft borders, subtle accents). **Brand Color Application System**: LayoutProvider dynamically applies organization-specific brand colors from `themeColors` JSONB field (primary, accent, background, foreground/text) as CSS variables (`--primary`, `--accent`, `--background`, `--foreground`) on document root. Robust color conversion using Canvas API handles all formats (hex, rgb, rgba, hsl, hsla) with safeguards to prevent partial theme updates on conversion failures. Colors scraped during provisioning or manually overridden flow from database → `/api/organization/current` → OrganizationContext → LayoutProvider for automatic application across entire platform. **AI-Powered Layout Recommendation Engine**: Google Gemini analyzes organization characteristics (name, mission, scraped content, personas, organization type) to suggest the best-fit layout with reasoning and confidence levels (high/medium/low). Multi-strategy JSON extraction (markdown fence stripping + first-to-last brace matching) handles diverse Gemini response formats. Heuristic fallback using keyword analysis (nature, community, modern, classic indicators) returns null when data is insufficient (< 20 chars). API endpoint (POST /api/admin/organizations/recommend-layout) validates input with Zod schema and returns 200 with null on graceful failures. Frontend displays recommendations in Step 3 with confidence-based styling (primary for high/medium, muted for low), auto-selects only medium/high confidence recommendations, and shows disclaimer for low-confidence suggestions. Recommendation triggers automatically after website scraping or when entering Step 3 for non-website strategies.
