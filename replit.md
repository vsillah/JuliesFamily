...
- **Organization-Scoped Storage** (`server/orgScopedStorage.ts`): JavaScript Proxy-based wrapper that intercepts ALL IStorage method calls (325 methods) and enforces tenant isolation. Methods are classified as: global (13 methods - user/org operations), implemented org-scoped (18 methods - leads, content, donations), or unimplemented (294 methods). Unimplemented methods throw security errors when called to prevent cross-tenant data leakage. Startup validation reports coverage statistics (currently 10%). Middleware (`server/orgStorageMiddleware.ts`) attaches org-scoped storage instance to each request via `req.storage`.
