...
- **Tenant Isolation Testing**: Manual test script (`tests/tenant-isolation.test.ts`) verifies isolation across 6 critical resource types. Tests confirm each org sees only its own data and cross-org access attempts are properly blocked.
...
- **Security Approach**: Hostname normalization, trusted domain validation, DNS verification for custom domains, request-scoped storage enforcement, Proxy-based hard enforcement prevents cross-tenant data leakage through HTTP routes.
