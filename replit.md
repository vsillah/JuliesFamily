...
-   **URL Mapping & Branding Extraction**: Enhanced provisioning wizard with optional URL mapping fields (programsUrl, eventsUrl, testimonialsUrl) for fine-grained content scraping from multi-page nonprofit websites. Automatic logo extraction (favicon, og:image, header logos) and theme color detection (primary, accent, background, text) from CSS/stylesheets during organization provisioning. Extracted branding data stored in organizations table (logo, themeColors JSONB) and displayed in provisioning wizard preview UI.
