...
- **Organization-Scoped Storage** (`server/orgScopedStorage.ts`): JavaScript Proxy-based wrapper that intercepts ALL IStorage method calls (325 methods) and enforces tenant isolation. Methods are classified as: global (13 methods - user/org operations), implemented org-scoped (87 methods), or unimplemented (241 methods). Unimplemented methods throw security errors when called to prevent cross-tenant data leakage. Startup validation reports coverage statistics (currently 31%). Middleware (`server/orgStorageMiddleware.ts`) attaches org-scoped storage instance to each request via `req.storage`.
  - **Comprehensive Org-Scoped Operations**: Leads (8), Content (6), Donations (4), Email Campaigns/Templates (10), SMS Campaigns/Templates (9), Email Tracking (6), Communication Logs (2), Interactions (2), Segments (5), Image Assets (5), Volunteer Events (5), Volunteer Enrollments (5), Volunteer Shifts (3), Volunteer Session Logs (3), Lead Assignments (3), Outreach Emails (3), Campaign Members (3), Scheduled Reports (5).
  - **Complete Route Migration (Nov 15, 2024)**: Migrated all 509 storage references in `server/routes.ts` from global `storage` to request-scoped `req.storage`. Fixed critical security breach in `server/twilio.ts` sendSMS function that was dynamically importing global storage. All helper functions (`email.ts`, `taskAutomation.ts`, `calendar.ts`, `twilio.ts`) and services (`cacLtgpAnalytics.ts`, `adminEntitlementService.ts`, `automationScheduler.ts`, `donorLifecycleService.ts`) now require injected `IStorage` parameter. Architect-approved: tenant isolation secure for all user-facing HTTP routes.
...
- **Security Features**: Hostname normalization (case-insensitive), trusted domain validation, custom domain verification requirement, error rejection (no unsafe defaults), request logging for security monitoring, Proxy-based hard enforcement of tenant isolation, request-scoped storage enforcement eliminates cross-tenant data leakage through HTTP routes.
