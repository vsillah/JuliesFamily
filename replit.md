...
- **Content Management System (CMS)**: A full-featured CMS for managing all website content, including images, visibility controls, drag-and-drop reordering, and integration with Cloudinary for image optimization. The Content Manager includes a "Show Hidden Items" toggle that allows administrators to view and manage both active and inactive content. When filtering hidden items (toggle OFF), drag-and-drop reordering is automatically disabled to prevent order corruption. All visibility toggle buttons include comprehensive accessibility attributes (aria-label, aria-pressed, data-testid) for improved usability and testability.
...
- **Social Media Feed**: Carousel-based display of curated social media posts with responsive layout (1/2/3 posts per viewport), self-contained data fetching, graceful handling of posts without images, and full Content Manager integration for visibility controls and content editing.
