...
const compactMarkers = [
  ...
  "sm:h-[min(360px,calc(100vh-19rem))]",
  "overflow-y-auto",
  "overflow-x-hidden",
  "lg:overflow-hidden",
  "overflow-hidden",
  ...
  "overflow-hidden rounded-lg border border-slate-200 bg-white p-2 sm:p-3",
  "rounded-lg border border-slate-200 bg-white p-2 shadow-sm sm:p-3",
  "section-kinflo-client-provisioning-summary-chips",
  "h-[min(360px,calc(100vh-16rem))]",
  "grid-cols-4",
  "mt-2 grid shrink-0 grid-cols-4 gap-1 sm:gap-2",
  "truncate text-[9px] font-medium uppercase tracking-normal text-slate-500 sm:text-[10px]",
  "flex min-h-0 flex-1 flex-col",
  "max-h-[190px]",
  "sm:max-h-[145px]",
  "value={clientWebsiteProvisioningView}",
  "setClientWebsiteProvisioningView(value as ClientWebsiteProvisioningView)",
  ...
  "max-h-[280px]",
  "grid-cols-[minmax(118px,0.82fr)_minmax(140px,1.18fr)]",
...
requireIncludes("docs/phase86-site-studio-scroll-consolidation.md", [
  ...
  "108px caps at narrow review widths",
  ...
  "capped at 340px on narrow review widths",
  "duplicate disabled gate buttons wait for desktop width",
  "local vertical scrolling rather than clipping content",
  "provisioning view is controlled by an explicit `Order` / `Dry run` sub-tab state",
